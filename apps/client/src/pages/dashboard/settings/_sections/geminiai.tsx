...
export const GeminiAISettings = () => {
  ...
  const onSubmit = ({ apiKey, baseURL, model, maxTokens }: Partial<FormValues>) => {
    setApiKey(apiKey as string | null);
    if (baseURL) {
      setBaseURL(baseURL);
    }
    if (model) {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h3 className="text-2xl font-bold leading-relaxed tracking-tight">{t`GeminiAi/Ollama Integration`}</h3>
        <p className="leading-relaxed opacity-75">
          {t`You can make use of the GeminiAi API to help you generate content, or improve your writing while composing your resume.`}
        </p>
      </div>
      ...
      <div className="prose prose-sm prose-zinc max-w-full dark:prose-invert">
        <p>
          <Trans>
            ...
            >
              obtain your own GeminiAi API key
            </a>
      ...
      <Form {...form}>
        <form className="grid gap-6 sm:grid-cols-2" onSubmit={form.handleSubmit(onSubmit)}>
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                <FormLabel>{t`GeminiAI/Ollama API Key`}</FormLabel>
                <FormControl>
      ...
      <div className="prose prose-sm prose-zinc max-w-full dark:prose-invert">
        <p>
          <Trans>
            ...
            making requests to GeminiAi via their official SDK. Rest assured that your key is not
            transmitted to any external server except when interacting with GeminiAi's services.
          </Trans>
        </p>
      </div>
      ...
      <Alert variant="warning">
        <div className="prose prose-neutral max-w-full text-xs leading-relaxed text-primary dark:prose-invert">
          <Trans>
            ...
            By utilizing the GeminiAi API, you acknowledge and accept the{" "}
            <a
            ...
            outlined by GeminiAi. Please note that Reactive Resume bears no responsibility for any
